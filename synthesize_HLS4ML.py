...
def synthezise():
  ...
  for i,layer in enumerate(model.layers):
    print('Layer n=',i,'     class=',layer.__class__.__name__,'      name=',layer.name)
  ...
  # Make HLS config
  config = hls4ml.utils.config_from_keras_model(model, granularity='name',default_reuse_factor=REUSE) #, default_precision='ap_fixed<24,12>'
...
#Special cases:      
  ...
  if changeStrategy: #Change strategy if layer is > 4,096. Doesn't work to set strategy per layer for io_stream models
      for layer in model.layers:
...
# Plot the NN model
  hls4ml.utils.plot_model(hls_model, show_shapes=True, show_precision=True, to_file="results/{}/hls_model_plot.png".format(mname))
  tf.keras.utils.plot_model(model,to_file='results/{}/keras_model_plot.png'.format(mname))


# Load data for inference
# data has shape (-1,nconstit,nfeat) for GNNs and (-1,nconstit*nfeat) for MLP
  nconstit = NCONST
  nfeat = 3
  X_test = np.load('./data/x_test_{}const.npy'.format(nconstit))
  Y_test = np.load('./data/y_test_{}const.npy'.format(nconstit), allow_pickle=True)

  if 'MLP' in mname: 
    X_test = np.reshape(X_test, (-1,nconstit*nfeat))
  else:
...
def getReports(indir):
    ...
    report_csynth = Path('results/{}/myproject_prj/solution1/syn/report/myproject_csynth.rpt'.format(indir))
